...
class Synthetic_data():
    ...
    def gibbs_sampler(self, max_iters=100):
        ...
        self.mu = [[gmm.cluster_centres]]
        self.sigma = [[gmm.cluster_covs]]
        self.r = [[np.linalg.norm(x) for x in self.normed_embds]]
        self.z = [gmm.labels]
        ...
        for t in range(max_iters):
            ...
            # Update r
            new_r = [self.sample_r_i(i, self.mu[-1][self.z[-1][i]], self.sigma[-1][self.z[-1][i]]) for i in range(self.N_t)]
            self.r.append(new_r)
...
if __name__ == '__main__':
    ...
    ds = Synthetic_data(μ_1, μ_2, 2, prior, N_t=1)

    ds.gibbs_sampler(max_iters=10)
